            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-6">
                        <div class="titulo_ban">Autorización de Requisiciones con Excedente de Presupuesto</div>
                    </div>
...
<script>
    ...
    var modulo='AUTORIZAR_PRESUPUESTO_REQUI';
    var idUnidadActual=<?php echo $_SESSION['id_unidad_actual']?>;
    var idsUnidades=',<?php echo $_SESSION['unidades']?>';//--MGFS SE AGREGA LA , PARA QUE IDENTIFIQUE LA UNIDADES
    ...
    $(function(){
       ...
       function buscarInformacion(idsSucursales){
            ...
            $.ajax({
                ...
                type: 'POST',
                url: 'php/autorizar_presupuesto_buscar.php',
                dataType:"json", 
                ...
                success: function(data) {
                   ...
                   if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var rechazada=(data[i].estatus=='Rehazada')?'checked':'';
                           
                            var editar=(data[i].editar=='no')?'disabled':'';
                            
                            var i_autorizar='<input type="checkbox" id="r_estatus_' + data[i].id+'" name="r_estatus_' + data[i].id+'" class="r_estatus" alt="'+data[i].id+'"  alt2="'+data[i].folio+'" value="1" '+autorizada+' '+editar+'>';
                            var i_rechazar='<input type="checkbox" id="r_estatus_' + data[i].id+'" name="r_estatus_' + data[i].id+'" class="r_estatus" alt="'+data[i].id+'" alt2="'+data[i].folio+'" value="2" '+rechazada+' '+editar+'>';
                            ...
                            var html='<tr class="renglon_autorizar '+data[i].estatus+'">\
                                       <td data-label="usuario">' + data[i].unidad_negocio+ '</td>\
                                        ...
                                        <td data-label="usuario">' + data[i].usuarioCapturo+ '</td>\
                                        <td data-label="usuario">' + data[i].fechaCreacion+ '</td>\
        ...
        $(document).on('click','#b_reactivar_guardar',function(){
            ...
            if(id>0){
                guardarEstatus(id,folio,0,'b_reactivar_guardar');
            }else{
        ...
        function guardarEstatus(id,folio,estatus,boton){
         $.ajax({
                type: 'POST',
                url: 'php/autorizar_presupuesto_guardar.php', 
                data: {
                   'id':  id,
                   'estatus' : estatus,
                   'idUsuario':idUsuario,
        ...
        $('#b_buscar_requis').on('click',function(){
            ...
            $.ajax({
                ...
                type: 'POST',
                url: 'php/autorizar_presupuesto_buscar_requis_reactivar.php',
                dataType:"json", 
                data:{'idsUnidades' : idsUnidades,
                      'idUsuario' : idUsuario,
        ...
        function muestraRegistro(idRequisicion){ 
            ...
            $.ajax({
                type: 'POST',
                url: 'php/autorizar_presupuesto_buscar_requis_reactivar_id.php',
                dataType:"json", 
                data:{
       ...
       function mandaGenerarPDF(idRequi){
        ...
        $('#b_excel').click(function(){
            ...
            $("#i_nombre_excel").val('Requisiciones fuera de Presupuesto');
            $("#i_fecha_excel").val(hoy);
            $('#i_modulo_excel').val('REPORTES_REQUISICIONES_FUERA_PRESUPUESTO');
