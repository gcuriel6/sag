...
<style> 
    ...
    #dialog_buscar_salidas_ajuste > .modal-lg{
        min-width: 80%;
        max-width: 80%;
    }
    .boton_eliminar{
            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-3">
                        <div class="titulo_ban">Salida Por Ajuste</div>
                    </div>
                ...
                <form id="forma_salida_ajuste" name="forma_salida_ajuste">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-4">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input type="text" id="i_tipo_salida" name="i_tipo_salida" value="S99 Salida Por Ajuste" class="form-control form-control-sm" autocomplete="off" readonly>
                                </div>
                            </div>
                            <div class="row">
                                <label for="s_clasificacion" class="col-sm-12 col-md-12 col-form-label">Clasificación a presupuesto </label>
                                <div class="col-sm-12 col-md-12">
                                    <select id="s_clasificacion" name="s_clasificacion" class="form-control validate[required]" autocomplete="off" style="width:100%;"></select>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <label for="i_observacion" class="col-sm-12 col-md-2 col-form-label requerido">Observación </label>
                        <div class="col-sm-12 col-md-9">
                            <input type="text" id="i_observacion" name="i_observacion" class="form-control validate[required]" autocomplete="off">
                        </div>
                    </div>
                </form>
                <br>
...
<div id="dialog_buscar_salidas_ajuste" class="modal fade bd-example-modal-lg"  role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Búsqueda de  Salidas Por Ajuste</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <input type="text" name="i_filtro_salidas_ajuste" id="i_filtro_salidas_ajuste" alt="renglon_salidas_ajuste" class="form-control filtrar_renglones" placeholder="Filtrar" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>    
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <table class="tablon"  id="t_salidas_ajuste">
                            <thead>
                                <tr class="renglon">
...
<script>
    ...
    var modulo='S_POR_AJUSTE';
    var idUnidadActual=<?php echo $_SESSION['id_unidad_actual']?>;
    var idUsuario=<?php echo $_SESSION['id_usuario']?>;
    var usuario='<?php echo $_SESSION['usuario']?>';
    var idSalidaAjuste = 0;
    ...
    $(function(){
        ...
        //Llena combo clasificacion a presupuesto INICIO
        $('#s_clasificacion').select2();

        $('#s_clasificacion').html('');
        var html='<option selected disabled>Selecciona</option>';
        $('#s_clasificacion').append(html);

        $.ajax({
            type: 'POST',
            url: 'php/combos_buscar.php',
            dataType:"json", 
            data:{'tipoSelect' : 'CLASIFICACION_SALIDAS_AJUSTES'},
            ...
            error: function (xhr) {
                console.log("php/combos_buscar.php-->"+JSON.stringify(xhr));    
                mandarMensaje('* No se encontró información de Clasificación a Presupuesto');
            }
        });
        //Llena combo clasificacion a presupuesto FIN
        ...
        $('#b_guardar').click(function(){
            ...
            if ($('#forma_salida_ajuste').validationEngine('validate')){
                if($('#t_registros .partida').length > 0)
                {
                    ...
                    mandarMensaje('Debe existir por lo menos un producto para generar la salida por ajuste');
        ...
        function guardar(){
            ...
            var datos = {
                ...
                'usuario':usuario,
                'idClasificacion':$('#s_clasificacion').val(),
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data > 0 )
                    { 
                        mandarMensaje('La salida por ajuste: '+data+' se guardó correctamente');
                        limpiar();
        ...
        $('#b_buscar').click(function(){
            ...
            $('#i_filtro_salidas_ajuste').val('');
            $('#i_fecha_inicio').val('');
            $('#i_fecha_fin').val('').prop('disabled',true);
            $('.renglon_salidas_ajuste').remove();
            $('#dialog_buscar_salidas_ajuste').modal('show');
        ...
        $(document).on('change','#s_filtro_sucursal',function(){
            buscarSalidasPorAjuste($('#s_filtro_unidad').val());
        });
        ...
        function buscarSalidasPorAjuste(idUnidadNegocio){
            $('.renglon_salidas_ajuste').remove();
                ...
                success: function(data) {
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var html='<tr class="renglon_salidas_ajuste" alt="'+data[i].id+'" alt2="'+data[i].id_unidad_negocio+'" alt3="'+data[i].id_sucursal+'">\
                                        <td data-label="No. Movimiento">' + data[i].folio+ '</td>\
                                        <td data-label="Sucursal">' + data[i].sucursal+ '</td>\
                                        <td data-label="Fecha">' + data[i].fecha+ '</td>\
                                        <td data-label="Observación">' + data[i].observacion+ '</td>\
                            ...
                            $('#t_salidas_ajuste tbody').append(html);   
        ...
        $('#t_salidas_ajuste').on('click', '.renglon_salidas_ajuste', function() {
            muestraSucursalesPermiso('s_id_sucursales',$(this).attr('alt2'),modulo,idUsuario);
            idSalidaAjuste = $(this).attr('alt');
            $('#dialog_buscar_salidas_ajuste').modal('hide');
            muestraRegistro(idSalidaAjuste);
            muestraRegistroDetalle(idSalidaAjuste); 
            $('#b_guardar,#b_agregar_producto,#b_buscar_productos').prop('disabled',true);
            $('#b_imprimir').prop('disabled',false);
            $('#forma_salida_ajuste').find('input,select').prop('disabled',true);
            $('#form_partidas input').prop('disabled',true);
        });

     

        function muestraRegistro(idSalidaAjuste){
            $.ajax({
                type: 'POST',
                ...
                success: function(data) {
                    if(data.length > 0){
                        ...
                        $('#i_observacion').val(data[0].observacion);
                        ...
                        if(data[0].id_clasificacion != 0)
                        {
                            $('#s_clasificacion').val(data[0].id_clasificacion);
                            $('#s_clasificacion').select2({placeholder: $(this).data('elemento')});
                        }else{
                            $('#s_clasificacion').val('');
                            $('#s_clasificacion').select2({placeholder: ''});
                        }
                    }
        ...
        function muestraRegistroDetalle(idSalidaAjuste){
                    ...
                    success: function(data)
                    {
                        ...
                        for(var i=0; data.length>i; i++)
                        {
                            ...
                            if(!row)
                            {
                                ...
                                //-->NJES April/23/2020 se agrega atributo servicio
                                var html = "<tr class='tr_producto' id='bp_" + producto.id + "' alt='" + producto.id + "'  alt2='" + producto.concepto+ "' alt3='" + producto.id_familia + "' alt4='" + producto.familia + "' alt5='" + producto.id_linea + "' alt6='" + producto.linea + "' alt7='" + producto.precio + "' alt8='" + producto.descripcion + "' alt9='"+producto.existencia+"' servicio='"+producto.servicio+"'>";
                                html += "<td data-label='Familia'>" + producto.familia + "</td>";
                                html += "<td data-label='Línea'>" + producto.linea + "</td>";
                                html += "<td data-label='Concepto'>" + producto.concepto + "</td>";
                                html += "<td align='right' data-label='Precio'>" + formatearNumero(producto.costo) +  "</td>";
        ...
        $('#b_imprimir').click(function(){
            ...
            var datos = {
                ...
                'idRegistro':idSalidaAjuste,
                'nombreArchivo':'salida_por_ajuste',
                'tipo':1,
                'concepto':'S99 Salida Por Ajuste'
            };
        ...
        function limpiar(){
            muestraSelectUnidades(matriz,'s_id_unidades',idUnidadActual);
            muestraSucursalesPermiso('s_id_sucursales',idUnidadActual,modulo,idUsuario);
            $('#i_num_movimiento,#i_observacion').val('');
            ...
            $('#s_clasificacion').val('');
            $('#s_clasificacion').select2({placeholder: 'Selecciona'});

            $('#forma_salida_ajuste').validationEngine('hide');
            $('#form_partidas').validationEngine('hide');
            $('#form_partidas input').prop('disabled',false);
            $('#i_observacion').prop('disabled',false);
