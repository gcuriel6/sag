...
<style> 
    ...
    #dialog_buscar_entradas_ajuste > .modal-lg{
        min-width: 80%;
        max-width: 80%;
    }
    .boton_eliminar{
            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-3">
                        <div class="titulo_ban">Entrada Por Produccion</div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-4">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input type="text" id="i_tipo_entrada" name="i_tipo_entrada" value="E08 Entrada Por Producción" class="form-control form-control-sm" autocomplete="off" readonly>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div id="dialog_buscar_entradas_ajuste" class="modal fade bd-example-modal-lg"  role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Búsqueda de  Entradas Por Producción</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <input type="text" name="i_filtro_entradas_ajuste" id="i_filtro_entradas_ajuste" alt="renglon_entradas_ajuste" class="form-control filtrar_renglones" placeholder="Filtrar" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>    
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <table class="tablon"  id="t_entradas_ajuste">
                            <thead>
                                <tr class="renglon">
...
<script>
    ...
    var modulo='E_POR_PRODUCCION';
    var idUnidadActual=<?php echo $_SESSION['id_unidad_actual']?>;
    var idUsuario=<?php echo $_SESSION['id_usuario']?>;
    var usuario='<?php echo $_SESSION['usuario']?>';
    var idEntradaAjuste = 0;

    let objetoMateriasPrimas=[];
    ...
    $(function(){
        ...
        $('#s_id_unidades').change(function(){
            ...
            let ruta = "php/buscar_productos_materia_prima.php";
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(json, function( index, value ) {
                        ...
                        let pt = value.pt.split(",");

                        $.each(pt, function( index, val ) {
                            let id = value.id;
                            let concepto = value.concepto;
                            let exis = value.existencia;
                            let canti = value.cantidad;

                            objetoMateriasPrimas.push({id, concepto, exis, "pt":val, canti});
                        });
                        //objetoMateriasPrimas
                ...
                error: function (xhr) 
                {
                    console.log(ruta+JSON.stringify(xhr));
        ...
        $('#b_guardar').click(function(){
            ...
            if ($('#forma_entrada_ajuste').validationEngine('validate')){
                if($('#t_registros .partida').length > 0)
                {
                    ...
                    mandarMensaje('Debe existir por lo menos un producto para generar la entrada por ajuste');
        ...
        function guardar(){
            ...
            //E08 = Entrada Por Produccion
            var datos = {
                'tipoEntrada':'E08',
                'idEntrada':idEntradaAjuste,
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data > 0 )
                    { 
                        mandarMensaje('La entrada por ajuste: '+data+' se guardó correctamente');
                        limpiar();
        ...
        $('#b_buscar').click(function(){
            ...
            $('#i_filtro_entradas_ajuste').val('');
            $('#i_fecha_inicio').val('');
            $('#i_fecha_fin').val('').prop('disabled',true);
            $('.renglon_entradas_ajuste').remove();
            $('#dialog_buscar_entradas_ajuste').modal('show');
        });
        ...
        $(document).on('change','#s_filtro_sucursal',function(){
            buscarEntradasPorAjuste($('#s_filtro_unidad').val());
        });
        ...
        function buscarEntradasPorAjuste(idUnidadNegocio){
            $('.renglon_entradas_ajuste').remove();
                ...
                success: function(data) {
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var html='<tr class="renglon_entradas_ajuste" alt="'+data[i].id+'" alt2="'+data[i].id_unidad_negocio+'" alt3="'+data[i].id_sucursal+'">\
                            ...
                            $('#t_entradas_ajuste tbody').append(html);   
        ...
        $('#t_entradas_ajuste').on('click', '.renglon_entradas_ajuste', function() {
            muestraSucursalesPermiso('s_id_sucursales',$(this).attr('alt2'),modulo,idUsuario);
            idEntradaAjuste = $(this).attr('alt');
            $('#dialog_buscar_entradas_ajuste').modal('hide');
            muestraRegistro(idEntradaAjuste);
            muestraRegistroDetalle(idEntradaAjuste); 
        ...
        function muestraRegistro(idEntradaAjuste){
            $.ajax({
                type: 'POST',
        ...
        function muestraRegistroDetalle(idEntradaAjuste){
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    for(var i=0; data.length>i; i++)
                    {
                        var html = "<tr class='partida_EA' producto='" + data[i].id_producto + "' concepto='" + data[i].concepto+ "' id_familia='" + data[i].id_familia + "' familia='" + data[i].familia + "' id_linea='" + data[i].id_linea + "' linea='" + data[i].linea + "' precio='" + data[i].precio + "' cantidad='" +  data[i].cantidad + "' descripcion='" + data[i].descripcion + "' importe='" + data[i].importe + "' marca='"+data[i].marca+"' talla='"+data[i].talla+"'>";
        ...
        $('#b_buscar_productos').click(function(){
            ...
            if($('#s_id_unidades').val() != null)
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'php/productos_buscar_activos_con_equivalentes.php',
                    dataType:"json", 
                    data:{
                        'idUnidad':idUnidad,
                        'idFamilia':0,
                    ...
                    error: function (xhr)
                    {
                        console.log('php/productos_buscar_activos.php-->'+JSON.stringify(xhr));
        ...
        $('#b_agregar_producto').click(function(){
            ...
            if($('#form_partidas').validationEngine('validate'))
            {
                ...
                let filtrado = objetoMateriasPrimas.filter(x => x.pt == idProducto);

                // console.log(filtrado);
                let validado = true;

                $.each(filtrado,(id, val)=>{
                    let total = val.canti * cantidad;
                ...
                if(validado == true){
                    ...
                    var html = "<tr class='partida' producto='" + idProducto + "' concepto='" + concepto+ "' id_familia='" + idFamilia + "' familia='" + familia + "' id_linea='" + idLinea + "' linea='" + linea + "' precio='" + precio + "' cantidad='" +  cantidad + "' descripcion='" + descripcion + "' importe='" + importe + "' marca='"+marca+"' talla='"+talla+"' existencia='"+existencia+"'>";
                    ...
                    mandarMensaje('Cantidad sobrepasa la existencia de materia prima.');
                }
        ...
        $('#b_imprimir').click(function(){
            ...
            var datos = {
                ...
                'idRegistro':idEntradaAjuste,
                'nombreArchivo':'entrada_por_ajuste',
                'tipo':1,
                'concepto':'E08 Entrada Por Produccion'
            };
