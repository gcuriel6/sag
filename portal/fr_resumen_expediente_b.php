			...
			for($i = 0; $i < $cont;$i++)
			{
				...
				if($accesos[$i][2] != 3)
				{
					...
					TablaQuery("SELECT a.id_trabajador AS id,CONCAT(a.apellido_p,' ',a.apellido_m,' ',a.nombre) AS 'nombre completo',b.puesto,c.des_dep AS 'departamento' ,c.cve_dep AS 'clave'
						...
						WHERE a.id_sucursal = '$suc' AND a.id_depto IN ($dep) AND fecha_baja != '0000-00-00' ",$link,"SUCURSAL $sucur","php/detalle_trabajador_b.php","id","nombre completo");
					//echo "con";
					}
					else
					{
					...
					TablaQuery("SELECT a.id_trabajador AS id,CONCAT(a.apellido_p,' ',a.apellido_m,' ',a.nombre) AS 'nombre completo',b.puesto,c.des_dep AS 'departamento' ,c.cve_dep AS 'clave'
						...
						LEFT JOIN cat_puestos b ON a.id_puesto = b.id_puesto
						LEFT JOIN deptos c ON a.id_depto = c.id_depto
						WHERE a.id_sucursal = '$suc' AND fecha_baja != '0000-00-00'",$link,"SUCURSAL $sucur","php/detalle_trabajador_b.php","id","nombre completo");
					//echo "sin";
					}
				}
			}
