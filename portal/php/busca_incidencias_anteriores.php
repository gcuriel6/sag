<?php
	...
	$query = 'SELECT * FROM vw_incidencias_full_quincena WHERE fecha < CURDATE() AND validado = 0 ORDER BY fecha,depto';

	if($_SESSION["id_usuario"] == 16){
		$query = 'SELECT * FROM vw_incidencias_full_quincena WHERE fecha < CURDATE() AND validado = 0 AND compania = "REAL SHINY SUCURSAL CHIHUAHUA" ORDER BY fecha,depto';
	}
