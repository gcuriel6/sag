<?php
	...
	$query2 = "SELECT nivel_web,permisos_web FROM accesos WHERE usuario = '$user' AND id_sucursal = $sucursal";
	...
	if($registro2 = mysqli_fetch_array($resultado2))
	{
		...
		$psico = checaPermisoForma('EXPEDIENTE_WEB', $user, $link);
		//echo '<script language="javascript"> alert('.$psico.'); </script>';  
	}
	else
	{
		//echo "<script language='javascript'>alert('no tienes acceso a esta informacion');window.open('home.php','_self');</script>";
	}
	//----------------------------------------
	...
	function checaPermisoForma($forma,$usuario,$link){
		...
		{
			//echo '<script language="javascript"> alert("entro nuevo"); </script>';
			return 0;
		}
	}

	/**
	 * MGFS 12-02-2020  SE AGREGAN PERMISOS EN MENU PARA VER LOS TR DE:
	 * VER PSICOSOMENTRICO ==1 PODRA VER DOCUMENTO PSICISOMENTRICO
	 * LIMITAR DOCUMENTOS ==0 PODRA VER CONTRATO INDETERMINADO, SOLICITUD DE EMPLEO, DOCUMENTO NUEVO INGRESO
	 */	 
	function buscarPermisos($idUsuario,$trPermiso,$idSucursal,$link){
      
		$verifica = 0;
  
		$queryM = "SELECT sistema,comando FROM menus WHERE menuid = '$trPermiso' ORDER BY orden";
		$resultM = mysqli_query($link, $queryM)or die(mysqli_error());
		...
		if($num > 0){
		  ...
		  if($numP > 0){
			...
			while($row = $resultP->fetch_assoc()){
			   ...
			   if(checaBit($permisoForma,$permisoUsuario)){
				  $verifica = 1;
			   }else{
				  $verifica = 0;
			   }
...
<style>
	...
	#tabla_activos_asignados > thead > tr >td{
		text-align:center;
		font-weight:bolder;
	...
	#tabla_activos_asignados > tbody > tr >td{
		text-align:center;
		background-color:white;
		color:black;
...
$(function() 
	{
		...
		var psico = <?php echo $psico?>;
		var archivo_cambio= 0;
		var permisoPsico=<?php echo buscarPermisos($idUsuario,'VER_PSICOMETRICO',$sucursal,$link)?>;
		var permisoLimitarD=<?php echo buscarPermisos($idUsuario,'LIMITAR_DOCUMENTOS',$sucursal,$link)?>;

		console.log('Nivel ' + nivel);
		console.log('Permisos ' + permisos);

		//alert(nivel+' - '+permisos+' - '+psico +'--'+permisoPsico+'--'+permisoLimitarD);
		...
		$('#upload_file').click(function(){
			...
			var fileSize = document.getElementById("i_files").files;

			if(fileSize[0].size <= 6000000)
			{
				$('#fondo_cargando').show();
				...
				alert("El tamaño maximo del archivo es de 6 Mb");
				$('#upload_file').prop('disabled',false);
			}
		});
...
<body>
	...
	<div class="vinetas">
		<ul>
			...
			?>
			<li><a href="#tabs-7">Activos Asignados</a></li>
		</ul>
			<form id="forma" action='editar_trabajador.php?id=<?php echo $id_trabajador ?>' method='post'>
					...
					<center>
						<table>
							<tr>
								...
								<td><p class="dato"><?php echo $registro['nombre'] ?></p><input name="nombre"  class="entradas" value="<?php echo $registro['nombre'] ?>"/></td>
							...
							<tr>
								...
								<td><p class="dato"><?php echo $registro['apellido_p'] ?></p><input name="apellido_p"  class="entradas" value="<?php echo $registro['apellido_p'] ?>"/></td>
							</tr>
							<tr>
								<td class='etiqueta'>Apellido Materno:</td>
								<td><p class="dato"><?php echo $registro['apellido_m'] ?></p><input name="apellido_m"  class="entradas" value="<?php echo $registro['apellido_m'] ?>"/></td>
							</tr>
							<tr>
					...
					<center>
						<table>
							<tr>
								...
								<td><p class="dato"><?php echo $registro['direccion'] ?></p><input name="direccion"  class="entradas" value="<?php echo $registro['direccion'] ?>"/></td>
							...
							<tr>
								...
								<td><p class="dato"><?php echo $registro['colonia'] ?></p><input name="colonia"  class="entradas" value="<?php echo $registro['colonia'] ?>"/></td>
							...
							<tr>
								...
								<td><p class="dato"><?php echo $registro['estado'] ?></p><select name="cve_estado" class="entradas" id="s_estados"></select></td>
							...
							<tr>
								...
								<td><p class="dato"><?php echo $registro['municipio'] ?></p><select name="cve_municipio"  class="entradas" id="s_municipios"></select></td>
					...
					<center>
						<table>
							...
							<tr>
								...
								<td><p class="dato"><?php echo $registro['nom_esposa'] ?></p><input name="nom_esposa"  class="entradas" value="<?php echo $registro['nom_esposa'] ?>"/></td>
								<td class="etiqueta">Trabajo Esposa:</td>
								<td><p class="dato"><?php echo $registro['trabajo_esposa'] ?></p><input name="trabajo_esposa"  class="entradas" value="<?php echo $registro['trabajo_esposa'] ?>"/></td>
							</tr>
							<tr>
				...
				<div id="tabs-4">
					<table style="width:60%;">
						...
						<!--MGFS 12-02-2020 SE AGREGA PERMISO PARA VER TR CONTRATO INDETERMINADO-->
						<tr class='<?php if(($nivel_user == '2') && (trim($row["contrato_ind"])=="no")) echo "no_mostrar";?> trDocs'>						
						...
						<!--MGFS 12-02-2020 SE AGREGA PERMISO PARA VER TR PSICOSOMETRICO-->
						

						<tr class='trPS <?php if(($nivel_user == '2') && (trim($row["examen_psicometrico"])=="no")) echo "no_mostrar";?>'>
						...
						<!--MGFS 12-02-2020 SE AGREGA PERMISO PARA VER TR SOLICITUD EMPLEO, -->
						

						<tr class='trDocs <?php if(($nivel_user == '2') && (trim($row["solicitud_empleo"])=="no")) echo "no_mostrar";?>'>
						...
						<!--MGFS 12-02-2020 SE AGREGA PERMISO PARA VER TR DOC NUEVO INGRESO-->
						

						<tr class='trDocs <?php if(($nivel_user == '2') && (trim($row["documento_nuevo_ingreso"])=="no")) echo "no_mostrar";?>'>
						...
						<!--NJES November/23/2020 se agrega permiso para tr Encuesta de Salida-->
						<tr class='trDocs <?php if(($nivel_user == '2') && (trim($row["encuesta_salida"])=="no")) echo "no_mostrar";?>'>
						<td class="etiqueta">Encuesta de Salida:</td>
						<td class="datos"><?php if(trim($row["encuesta_salida"])!="no")echo "Capturada"; else echo "Sin Capturar"; ?></td>
						<!--<td><?php if(trim($row["encuesta_salida"])!="no") echo "<img class='docs' src='../images/icono-pdf.png' width='40' alt='../../../Portal/".  substr($row["encuesta_salida"], strpos($row["encuesta_salida"], 'docs'))  ."' /><button class='boton entradas borrar' title='encuesta_salida'>borrar</button>"; ?></td>-->
						<td><?php if(trim($row["encuesta_salida"])!="no") echo "<img class='docs' src='../images/icono-pdf.png' width='40' alt='../../../Portal/".  substr($row["encuesta_salida"], strpos($row["encuesta_salida"], 'docs'))  ."' /><button class='boton entradas borrar' title='encuesta_salida'>borrar</button>"; ?></td>
						</tr>

						<!-- 28/Junio/2022 GCM se agregan 7 campos -->
				...
				<div id="tabs-7">
					<center>
						<table style="width:100%;" class="striped" id="tabla_activos_asignados">
							<thead>
								<tr>
									<td>No. Económico</td>
									<td>No. Serie</td>
							...
							<tbody>
							<?php 
								$query_at = "SELECT DATE(fecha_inicio) AS fecha_asignacion,
									b.no_serie AS num_serie,
									b.num_economico,
									b.descripcion,
									CASE
										WHEN b.tipo = 1 THEN 'Vehiculo'
										WHEN b.tipo = 2 THEN 'Celular'
										WHEN b.tipo = 3 THEN 'Equipo de Computo'
										WHEN b.tipo = 5 THEN 'Armas'
										ELSE 'Otro'
									END AS tipo,
									IF(b.tipo=1,IFNULL(d.imei_gps,''),IF(b.tipo=2,IFNULL(c.imei,''),'')) AS num_imei
									FROM activos_responsables a
									LEFT JOIN activos b ON a.id_activo=b.id
									LEFT JOIN celulares c ON b.id=c.id_activo
									LEFT JOIN vehiculos d ON b.id=d.id_activo
									WHERE a.id_trabajador =".$id_trabajador." AND a.responsable = 1";

								$resul_at = mysqli_query($link, $query_at);
								while($row_at = mysqli_fetch_array($resul_at))
								{
									echo "<tr>";
									echo "<td>".$row_at['num_economico']."</td>";
									echo "<td>".$row_at['num_serie']."</td>";
									echo "<td>".$row_at['num_imei']."</td>";
									echo "<td>".$row_at['tipo']."</td>";
									echo "<td>".$row_at['descripcion']."</td>";
									echo "<td>".$row_at['fecha_asignacion']."</td>";
									echo "</tr>";
								}
							?>
							</tbody>
	...
	<div id="d_subir_doc">
		<center>
			<form id="form_upload" action="subir_archivo.php" method="post" enctype="multipart/form-data">
				...
				<h2>* El tamaño maximo de los archivos es de 6Mb</h2>
