            ...
            <div class="form-row">
              ...
              <div class="form-row">
                <div class="form-group col-md-6">
                  ...
                  <input type="text" class="form-control" id="totalProspecto">
                  <div class="invalid-feedback">
                    Campo con informacion no valida. 
                  </div>
                </div>
...
<script>
  ...
  $("#btnAgregarProductoExistente").on("click",()=>{
    ...
    let costo = $('option:selected', "#nombreProductoExistente").attr('alt2');
  ...
  $("#btnAprobarCotizacion").on("click",()=>{
    ...
    if(idCliente){
      cambiarEstatusCotizacion(idCotiz, idCliente, 1);
    }else{
  ...
  $("#tableListadoHistorial").on("click",".btnCancelar",function(){
    let id = $( this ).closest( "tr" ).attr("alt");
    cambiarEstatusCotizacion(id, 0, 2);
  });
  ...
  let traerProductos = () => {
    ...
    $.when(prods,maters,historial, clientes).done(function(p1, m1, h1, c1) {
      ...
      p1[0].map((k,v, i)=>{
        $("#nombreProductoExistente").append(`<option value="${k.idProducto}" alt1="${k.nombre}" alt2="${k.costo}" alt3="${k.clave}">${k.nombre} - ${k.costo}</option>`);
      });

      m1[0].map((k,v, i)=>{
        $("#nombreMateriaExistente").append(`<option value="${k.idMateria}" alt1="${k.descr}" alt2="${k.clave}" >${k.descr} - ${k.clave}</option>`);
      });
      ...
      h1[0].map((k,v, i)=>{
        ...
        let btnEstatus =`<button class="btn btn-success btn-sm btnAprobar"><i class="fas fa-check-circle"></i></button><button class="btn btn-warning btn-sm btnCancelar"><i class="fas fa-ban"></i></button>`;
  ...
  let cambiarEstatusCotizacion = (idCotiz, idCliente, estatus) => {
    $.ajax({
      type: 'POST',
      url: 'php/vision_estatus_cotizacion.php',
      data: {idCotiz, idCliente, estatus},
      dataType:"json"
    }).done(function(data) {
      if(data>0){
        $('.modal').modal('hide');
