...
function mandarMensajeConfimacion(mensaje,id_registro,concepto){
    ...
    $("#modal_alerta2 .modal-footer").html("<button type='button' class='btn btn-danger b_cancelar' data-dismiss='modal'>Cancelar</button><button id='b_"+concepto+"' alt='"+id_registro+"' type='button' class='btn btn-primary b_aceptar' data-dismiss='modal'>Continuar</button>");
...
*/
function muestraAreasAcceso(idSelect)
{
...
function muestraSelectSalariosPuestos(contenedor,idUnidadNegocio){
    $('#'+contenedor).select2();
...
function mostrarBotonAyuda(forma){
    $.ajax({
        ...
        success: function(data)
        {
            ...
            for(var i=0;data.length>i;i++)
            {
                var boton_a='<span class="boton_modal_ayuda" id="boton_ayuda" alt="'+forma+'" alt2="'+data[i].boton+'" aria-hidden="true"><i class="fa fa-info-circle" aria-hidden="true"></i></span>';
...
function muestraCuentasBancos(idSelect, idCuentaOrigen,tipo)
{
    ...
    $('#'+idSelect).html('');
    var html='<option selected disabled>Selecciona un Banco</option>';
    $('#'+idSelect).append(html);
...
function muestraTiposGasto(idSelect)
{
    ...
    $('#'+idSelect).html('');
    var html='<option selected disabled>Elige un Tipo</option><option value="F">Factura</option><option value="N">Nota</option><option value="R">Recibo</option>';
        ...
        success: function(data) {
            ...
            if(data!=0){
                ...
                for(var i=0;i<arreglo.length;i++){
                    ...
                    html+="<option value="+dato.id+" alt="+dato.dias_cred+" alt2="+dato.rfc+" alt3="+dato.codigo_postal+" alt4='"+dato.correo_facturas+"'>"+dato.razon_social+"</option>";
                }
                $('#'+contenedor).html(html);
        ...
        success: function(data) {
            ...
            if(data!=0){
                ...
                for(var i=0;i<arreglo.length;i++){
                    ...
                    html+="<option value="+dato.clave+">"+dato.descripcion+"</option>";
                }
                $('#'+contenedor).html(html);
