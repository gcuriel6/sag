...
<style> 
    ...
    #dialog_buscar_salidas_proveedor > .modal-lg{
        min-width: 80%;
        max-width: 80%;
    }
    .boton_eliminar{
            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-4">
                        <div class="titulo_ban">Salida Por Devolución a Proveedor</div>
                    </div>
                ...
                <form id="forma_salida_proveedor" name="forma_salida_proveedor">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-4">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input type="text" id="i_tipo_salida" name="i_tipo_salida" value="S04 Salida Por Devolución a Proveedor" class="form-control form-control-sm" autocomplete="off" readonly>
                                </div>
                            </div>
                            <div class="row">
                                <label for="i_proveedor" class="col-sm-12 col-md-12 col-form-label requerido">Proveedor </label>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-10">
                            <input type="text" id="i_observaciones" name="i_observaciones" class="form-control validate[required]" autocomplete="off">
                        </div>
                    </div>
                </form>
...
<div id="dialog_buscar_salidas_proveedor" class="modal fade bd-example-modal-lg" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Búsqueda de  Salidas Por Devolución a Proveedor</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <input type="text" name="i_filtro_salidas_proveedor" id="i_filtro_salidas_proveedor" alt="renglon_salidas_proveedor" class="form-control filtrar_renglones" placeholder="Filtrar" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>    
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <table class="tablon"  id="t_salidas_proveedor">
                            <thead>
                                <tr class="renglon">
...
<script>
    ...
    var modulo='S_POR_PROVEEDOR';
    var idUnidadActual=<?php echo $_SESSION['id_unidad_actual']?>;
    var idUsuario=<?php echo $_SESSION['id_usuario']?>;
    var usuario='<?php echo $_SESSION['usuario']?>';
    var idSalidaProveedor = 0;
    ...
    $(function(){
        ...
        $('#b_guardar').click(function(){
            ...
            if ($('#forma_salida_proveedor').validationEngine('validate')){
                if($('#t_registros .partida').length > 0)
                {
                    ...
                    mandarMensaje('Debe existir por lo menos un producto para generar la salida por proveedor de sucursal');
        ...
        function guardar(){
            ...
            //S04 = Salida Por Devolución a Proveedor
            var datos = {
                'tipoSalida':'S04',
                'idSalida':idSalidaProveedor,
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data > 0 )
                    { 
                        mandarMensaje('La salida por devolución de proveedor: '+data+' se guardó correctamente');
                        limpiar();
        ...
        $('#b_buscar').click(function(){
            ...
            $('#i_filtro_salidas_proveedor').val('');
            $('#i_fecha_inicio').val('');
            $('#i_fecha_fin').val('').prop('disabled',true);
            $('.renglon_salidas_proveedor').remove();
            $('#dialog_buscar_salidas_proveedor').modal('show');
        ...
        $(document).on('change','#s_filtro_sucursal',function(){
            buscarSalidasPorProveedor($('#s_filtro_unidad').val());
        });
        ...
        function buscarSalidasPorProveedor(idUnidadNegocio){
            $('.renglon_salidas_proveedor').remove();
                ...
                success: function(data) {
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            ///llena la tabla con renglones de registros
                            var html='<tr class="renglon_salidas_proveedor" alt="'+data[i].id+'" alt2="'+data[i].id_unidad_negocio+'" alt3="'+data[i].id_sucursal+'">\
                            ...
                            ///agrega la tabla creada al div 
                            $('#t_salidas_proveedor tbody').append(html);   
        ...
        $('#t_salidas_proveedor').on('click', '.renglon_salidas_proveedor', function() {
            muestraSucursalesPermiso('s_id_sucursales',$(this).attr('alt2'),modulo,idUsuario);
            idSalidaProveedor = $(this).attr('alt');
            $('#dialog_buscar_salidas_proveedor').modal('hide');
            muestraRegistro(idSalidaProveedor);
            muestraRegistroDetalle(idSalidaProveedor); 
            $('#b_guardar,#b_agregar_producto,#b_buscar_productos,#b_buscar_proveedor').prop('disabled',true);
            $('#b_imprimir').prop('disabled',false);
            $('#forma_salida_proveedor').find('input,select').prop('disabled',true);
            $('#form_partidas input').prop('disabled',true);
        });

        function muestraRegistro(idSalidaProveedor){
            $.ajax({
                type: 'POST',
        ...
        function muestraRegistroDetalle(idSalidaProveedor){
        ...
        $('#b_imprimir').click(function(){
            ...
            var datos = {
                ...
                'idRegistro':idSalidaProveedor,
                'nombreArchivo':'salida_por_proveedor',
                'tipo':1,
                'concepto':'S04 Salida Por Devolución a Proveedor'
            };
        ...
        function limpiar(){
            muestraSelectUnidades(matriz,'s_id_unidades',idUnidadActual);
            muestraSucursalesPermiso('s_id_sucursales',idUnidadActual,modulo,idUsuario);
            $('#i_num_movimiento,#i_observaciones,#i_proveedor,#i_supervisor').val('');

            $('#form_partidas input').val('');
            $('select').prop('disabled',false);
            $("#t_registros tr").remove();
            $('#b_guardar,#b_agregar_producto,#b_buscar_productos,#b_buscar_proveedor').prop('disabled',false);

            $('#forma_salida_proveedor').validationEngine('hide');
            $('#form_partidas').validationEngine('hide');
            $('#form_partidas input').prop('disabled',false);
            $('#b_imprimir').prop('disabled',true);
            $('#i_observaciones').prop('disabled',false);
        }
