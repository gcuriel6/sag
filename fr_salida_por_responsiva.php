...
<style> 
    ...
    #dialog_buscar_salidas_responsiva > .modal-lg{
        min-width: 80%;
        max-width: 80%;
    }
    .boton_eliminar{
            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-4">
                        <div class="titulo_ban">Salida Por Responsiva</div>
                    </div>
                ...
                <form id="forma_salida_responsiva" name="forma_salida_responsiva">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 col-md-4">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input type="text" id="i_tipo_salida" name="i_tipo_salida" value="S05 Salida Por Responsiva" class="form-control form-control-sm" autocomplete="off" readonly>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-5">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" id="i_importe" name="i_importe" class="form-control form-control-sm validate[required,custom[number]]" readonly autocomplete="off"/>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-12 col-md-2">
...
<div id="dialog_buscar_salidas_responsiva" class="modal fade bd-example-modal-lg" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Búsqueda de  Salidas Por Responsiva</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <input type="text" name="i_filtro_salidas_responsiva" id="i_filtro_salidas_responsiva" alt="renglon_salidas_responsiva" class="form-control filtrar_renglones" placeholder="Filtrar" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>    
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-12">
                        <table class="tablon"  id="t_salidas_responsiva">
                            <thead>
                                <tr class="renglon">
...
<script>
    ...
    var modulo='S_POR_RESPONSIVA';
    var idUnidadActual=<?php echo $_SESSION['id_unidad_actual']?>;
    var idUsuario=<?php echo $_SESSION['id_usuario']?>;
    var usuario='<?php echo $_SESSION['usuario']?>';
    var idSalidaResponsiva = 0;
    ...
    $(function(){
        ...
        $('#b_buscar_empleados').click(function(){
            if(($('#s_id_unidades').val() != null))
            {
                ...
                //-->NJES March/20/2020 se envia lista de las sucursales a las que tiene permiso el usuario en el modulo sin importar la unidad seleccionada
        ...
        $('#t_empleados').on('click', '.renglon_empleado', function() {
            ...
            var idDepartamento = $(this).attr('departamento');
            var idArea = $(this).attr('area');
            $('#i_empleado').attr({'alt':id,'depto':idDepartamento,'area':idArea}).val(id+' - '+nombre);
            $('#dialog_empleados').modal('hide');
        });

        $('#b_buscar_productos').click(function(){
            $('#i_familia_filtro,#i_linea_filtro,#i_filtro_producto').val('');
            ...
            if(($('#s_id_unidades').val() != null) && ($('#s_id_sucursales').val() != null))
            {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'php/productos_buscar_activos_existencia_sin_af.php',
                    dataType:"json", 
                    data:{
                        'idUnidad':idUnidad,
                    ...
                    success: function(data)
                    {
                        ...
                        for(var i=0; data.length>i; i++)
                        {
                            ...
                            //-->NJES Feb/10/2020 se agrega proiedad para obtener el id familia gasto del producto
                    ...
                    error: function (xhr)
                    {
                        console.log('php/productos_buscar_activos_existencia_sin_af.php-->'+JSON.stringify(xhr));
        ...
        $("#t_productos").on('click','.tr_producto',function(){
            ...
            //if(productosAgregados(idProducto)==''){ //NJES Jan/06/2020 permitir agregar un mismo producto para con otra talla
                ...
                //-->NJES Feb/10/2020 se agrega proiedad para obtener el id familia gasto del producto
                var idFamiliaGasto = renglon.attr('familia_gasto');
        ...
        function buscaFamilias(){
            ...
            $.ajax({
                ...
                type: 'POST',
                url: 'php/familias_buscar_no_tipo_activo_fijo.php',
                dataType:"json", 
                data:{'estatus':0},
        ...
        $('#b_guardar').click(function(){
            ...
            if ($('#forma_salida_responsiva').validationEngine('validate')){
                if($('#t_registros .partida').length > 0)
                {
                    ...
                    mandarMensaje('Debe existir por lo menos un producto para generar la salida por responsiva de sucursal');
        ...
        function guardar(){
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data > 0 )
                    { 
                        mandarMensaje('La salida por responsiva: '+data+' se guardó correctamente');
                        limpiar();
        ...
        $('#b_buscar').click(function(){
            ...
            $('#i_filtro_salidas_responsiva').val('');
            $('#i_fecha_inicio').val('');
            $('#i_fecha_fin').val('').prop('disabled',true);
            $('.renglon_salidas_responsiva').remove();
            $('#dialog_buscar_salidas_responsiva').modal('show');
        ...
        $(document).on('change','#s_filtro_sucursal',function(){
            buscarSalidasPorResponsiva($('#s_filtro_unidad').val());
        });
        ...
        function buscarSalidasPorResponsiva(idUnidadNegocio){
            $('.renglon_salidas_responsiva').remove();
                ...
                success: function(data) {
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            ///llena la tabla con renglones de registros
                            var html='<tr class="renglon_salidas_responsiva" alt="'+data[i].id+'" alt2="'+data[i].id_unidad_negocio+'" alt3="'+data[i].id_sucursal+'">\
                            ...
                            ///agrega la tabla creada al div 
                            $('#t_salidas_responsiva tbody').append(html);   
        ...
        $('#t_salidas_responsiva').on('click', '.renglon_salidas_responsiva', function() {
            muestraSucursalesPermiso('s_id_sucursales',$(this).attr('alt2'),modulo,idUsuario);
            idSalidaResponsiva = $(this).attr('alt');
            $('#dialog_buscar_salidas_responsiva').modal('hide');
            muestraRegistro(idSalidaResponsiva);
            muestraRegistroDetalle(idSalidaResponsiva); 
            $('#b_guardar,#b_agregar_producto,#b_buscar_productos,#b_buscar_empleados').prop('disabled',true);
            $('#b_imprimir').prop('disabled',false);
            $('#forma_salida_responsiva').find(' input,select').prop('disabled',true);
            $('#form_partidas input').prop('disabled',true);
        });

        function muestraRegistro(idSalidaResponsiva){
            $.ajax({
                type: 'POST',
        ...
        function muestraRegistroDetalle(idSalidaResponsiva){
        ...
        $('#b_imprimir').click(function(){
            ...
            var datos = {
                ...
                'idRegistro':idSalidaResponsiva,
                'nombreArchivo':'salida_por_responsiva',
                'tipo':1,
                'concepto':'S05 Salida Por Responsiva'
            };
        ...
        function limpiar(){
            ...
            $('#forma_salida_responsiva').validationEngine('hide');
            $('#form_partidas').validationEngine('hide');
