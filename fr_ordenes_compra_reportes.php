...
<style> 
    ...
    #div_t_detalle_orden_compra,#div_t_orden_compra_agrupadas,#div_t_orden_compra_agrupadas{
        max-height:220px;
        overflow:auto;
    }
    .titulo_tabla{
            ...
            <br>
                <div class="row">
                    <div class="col-sm-12 col-md-3">
                        <div class="titulo_ban">Reportes Órdenes de Compra</div>
                    </div>
                    ...
                    <div class="col-sm-12 col-md-1"></div>
                    <div class="col-sm-12 col-md-3">
                        <button type="button" class="btn btn-success btn-sm form-control verificar_permiso" alt="BOTON_EXCEL_REPORTES_COMPRAS" id="b_excel"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Excel</button>
                    </div>
                </div>
                ...
                <div class="row" id="div_detalle_orden_compra">
                    <div class="col-sm-12 col-md-12">
                        <div class="titulo_tabla">Detalle de Compras</div>
                        <table class="tablon">
                            <thead>
                                <tr class="renglon">
                        ...
                        <div id="div_t_detalle_orden_compra">
                            <table class="tablon"  id="t_detalle_orden_compra">
                                <tbody>
                ...
                <div class="row" id="div_agrupadas_orden_compra">
                    <div class="col-sm-12 col-md-12">
                        <div class="titulo_tabla">Ordenes de Compra Agrupadas</div>
                        <table class="tablon">
                            <thead>
                                <tr class="renglon">
                                    <th scope="col">Sucursal</th>
                                    ...
                                    <th scope="col">Abonos Anticipo</th>
                                </tr>
                            </thead>
                        </table>
                        <div id="div_t_orden_compra_agrupadas">
                            <table class="tablon"  id="t_orden_compra_agrupadas">
                                <tbody>
                ...
                <div class="row" id="div_backorder_orden_compra">
                    <div class="col-sm-12 col-md-12">
                        <div class="titulo_tabla">Backorder de Compras</div>
                        <table class="tablon">
                            <thead>
                                <tr class="renglon">
                                    <th scope="col">Sucursal</th>
                        ...
                        <div id="div_t_backorder_orden_compra">
                            <table class="tablon"  id="t_backorder_orden_compra">
                                <tbody>
...
<script>
    ...
    $(function(){
        ...
        $('#div_detalle_orden_compra').css('display','none');
        $('#div_agrupadas_orden_compra').css('display','none');
        $('#div_backorder_orden_compra').css('display','none');
        ...
        $('#s_id_unidades').change(function(){
            ...
            $('#t_detalle_orden_compra .renglon').remove();
        ...
        function seleccionaReporte(idUnidadNegocio){
            ...
            {  
                $('#div_detalle_orden_compra').css('display','block');
                $('#div_agrupadas_orden_compra').css('display','none');
                $('#div_backorder_orden_compra').css('display','none');
                buscaDetalleCompras(idUnidadNegocio);
                nombreReporte='Detalle de Compras';
            }else if($("#r_agrupadas").is(':checked'))
            {  
                $('#div_agrupadas_orden_compra').css('display','block');
                $('#div_detalle_orden_compra').css('display','none');
                $('#div_backorder_orden_compra').css('display','none');
                buscaComprasAgrupadas(idUnidadNegocio);
                nombreReporte='Ordenes de Compra Agrupadas';
            } else{
                $('#div_backorder_orden_compra').css('display','block');
                $('#div_agrupadas_orden_compra').css('display','none');
                $('#div_detalle_orden_compra').css('display','none');
                buscaBackorderCompras(idUnidadNegocio);
                nombreReporte='Backorder de Compras';
            } 
        }

        function buscaDetalleCompras(idUnidadNegocio){
            $('#t_detalle_orden_compra .renglon').remove();
            ...
            var datos = {
                ...
                'tipoReporte':'detalleCompras'
            }; 

            $.ajax({

                type: 'POST',
                url: 'php/orden_compra_reportes_buscar.php',
                dataType:"json", 
                data:{'datos':datos},
                    ...
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var html='<tr class="renglon">\
                                        ...
                                        <td data-label="Catálogo">'+data[i].clave+'</td>\
                                        ...
                                        <td data-label="% Dcto">'+formatearNumero(data[i].descuento)+'</td>\
                            ...
                            $('#t_detalle_orden_compra tbody').append(html);   
                ...
                error: function (xhr) 
                {
                    console.log('php/orden_compra_reportes_buscar.php --> '+JSON.stringify(xhr));
                    mandarMensaje('Error en el sistema');
                }
            });
        }

        function buscaComprasAgrupadas(idUnidadNegocio){
            $('#t_orden_compra_agrupadas .renglon').remove();
            ...
            var datos = {
                ...
                'idDepartamento':$('#s_id_departamento').val(),
                'idProveedor':$('#i_proveedor').attr('alt'),
                'tipoReporte':'ordenCompraAgrupadas'
            }; 
                    ...
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var html='<tr class="renglon">\
                                        ...
                                        <td data-label="Importe sin IVA">'+formatearNumero(data[i].importe_sin_iva)+'</td>\
                                        <td data-label="Importe con IVA">'+formatearNumero(data[i].importe_con_iva)+'</td>\
                                        <td data-label="Monto Anticipo">'+data[i].monto_anticipo+'</td>\
                                        <td data-label="Abonos Anticipo">'+data[i].abonos_anticipo+'</td>\
                                    </tr>';
                            ///agrega la tabla creada al div 
                            $('#t_orden_compra_agrupadas tbody').append(html);   
        ...
        function buscaBackorderCompras(idUnidadNegocio){
            $('#t_backorder_orden_compra .renglon').remove();
                    ...
                    if(data.length != 0){
                        ...
                        for(var i=0;data.length>i;i++){
                            ...
                            var html='<tr class="renglon">\
                                        ...
                                        <td data-label="Concepto">'+data[i].concepto+'</td>\
                                        <td data-label="Descripción">'+data[i].descripcion+'</td>\
                                        <td data-label="Detalle">'+data[i].cantidad+'</td>\
                                        <td data-label="Costo U">'+formatearNumero(data[i].costo_unitario)+'</td>\
                                        <td data-label="% Dcto">'+formatearNumero(data[i].descuento)+'</td>\
                                        <td data-label="% IVA">'+formatearNumero(data[i].porcentaje_iva)+'</td>\
                                        <td data-label="Importe sin IVA">'+formatearNumero(data[i].importe_sin_iva)+'</td>\
                            ...
                            ///agrega la tabla creada al div 
                            $('#t_backorder_orden_compra tbody').append(html);   
        ...
        $('#b_excel').click(function(){
            ...
            $('#i_modulo_excel').val('REPORTES_ORDEN_COMPRAS');
